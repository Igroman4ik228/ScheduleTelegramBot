...
class DefaultScheduleRepository(BaseRepositoryAlchemy[DefaultScheduleModel]):
    ...
    async def create(
        self, weekday: int, shift: int, data_lessons: str, group_id: int
    ...
    async def get(
        self, weekday: int, shift: int, group_id: int, *options
    ) -> DefaultScheduleModel | None:
        return await super().get(
            weekday=weekday, shift=shift, group_id=group_id, *options
        )

    async def get_by_group_name(
        self, weekday: int, shift: int, group_name: str, *options
    ) -> DefaultScheduleModel | None:
        group = await self.group_repo.get_by_name(group_name)
        if group is None:
        ...
        return await super().get(
            weekday=weekday, shift=shift, group_id=group.id, *options
        )

    async def delete(self, weekday: int, shift: int, group_id: int):
        await super().delete(weekday=weekday, shift=shift, group_id=group_id)
    ...
    async def delete_by_group_name(
        ...
        await super().delete(weekday=weekday, shift=shift, group_id=group.id)
