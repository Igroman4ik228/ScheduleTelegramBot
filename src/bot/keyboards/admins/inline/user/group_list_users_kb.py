...
class GroupCallbackFactory(
    CallbackData, prefix=CallbackDataAdmin.GROUP_LIST_USERS.value
):
    group_id: int | None = None


def get_group_kb(
    groups: list[GroupModel], show_no_group: bool = False
    ...
    if show_no_group:
        group_builder.add(
            InlineKeyboardButton(
                text="Без группы", callback_data=GroupCallbackFactory().pack()
            )
        )
