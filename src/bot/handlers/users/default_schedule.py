...
@router.callback_query(
    F.data == CallbackData.WRITE_DEFAULT_NUMERATOR_SCHEDULE.value
)
...
@router.callback_query(
    F.data == CallbackData.WRITE_DEFAULT_DENOMINATOR_SCHEDULE.value
)
...
async def get_default_schedule(
    group_id: int, repository: Repository, shift: int
) -> str:
    default_schedule_repo = repository.default_schedule
    default_schedule_data = await default_schedule_repo.get_all(
        shift=shift, group_id=group_id
    )
