...
class ScheduleNotifier(Observer):
    ...
    async def _update(
        ...
        *,
        ...
        self.logger.info("Start ScheduleNotifier: study_shift=%s, week=%s", study_shift, week)

        notification_recipients = await users.list_notification_recipients()

        for user in notification_recipients:
            if not self._should_notify(user, study_shift):
                continue
    ...
    def _should_notify(self, user: UserWithGroupSchema, study_shift: StudyShift) -> bool:
        if user.subscribe_id is None:
            return False
        return user.group is not None and user.group.study_shift == study_shift
