...
- Do not call `session.refresh()` after every ORM creation. Use it before serializing or reading DB-generated values (`server_default`, triggers, computed columns); refresh only needed attributes when possible.
